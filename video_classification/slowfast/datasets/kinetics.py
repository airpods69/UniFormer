...
class Kinetics(torch.utils.data.Dataset):
    ...
    def _construct_loader(self):
        ...
        path = '/shared/home/v_varenyam_bhardwaj/local_scratch/UniFormer/video_classification/data_list/k400'
        path_to_file = os.path.join(
            path, "{}_cleaned.csv".format(self.mode)
        )
                ...
                for idx in range(self._num_clips):
                    if self.mode == "test":
                        self._path_to_videos.append(
                            os.path.join(self.cfg.DATA.PATH_PREFIX, "val", path)
                        )
                    else:
                        self._path_to_videos.append(
                            os.path.join(self.cfg.DATA.PATH_PREFIX, f"{self.mode}", path)
                        )
